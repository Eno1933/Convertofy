...
import { 
  ...
  Trash2, MoveUp, MoveDown, Download 
} from 'lucide-react';
...
const MergeTool = ({ onBack }) => {
  ...
  const handleMerge = async () => {
    if (files.length < 2) {
      setError(t('merge.errorMinFiles'));
      return;
    }
    setProcessing(true);
    setProgress(0);
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-surface-lowest rounded-2xl shadow-ambient overflow-hidden">
          <div className="p-8 border-b border-outline-variant/20">
            <h1 className="font-display text-3xl md:text-4xl font-bold text-on-surface">{t('merge.title')}</h1>
            <p className="text-on-surface-variant mt-2">{t('merge.description')}</p>
          </div>
          ...
          <div className="p-8">
            ...
            >
              ...
              <p className="text-on-surface font-medium">{t('merge.dropzone')}</p>
              <p className="text-on-surface-variant text-sm mt-1">{t('merge.orClick')}</p>
              <p className="text-xs text-on-surface-variant/70 mt-4">{t('merge.maxSize')}</p>
            </div>

            {files.length > 0 && (
              <div className="mt-8">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="font-semibold text-on-surface">{t('merge.filesToMerge')} ({files.length})</h3>
                  <button
                    onClick={() => setFiles([])}
                    className="text-sm text-on-surface-variant hover:text-red-500 transition-colors flex items-center gap-1"
                  >
                    <Trash2 size={14} /> {t('merge.clearAll')}
                  </button>
                </div>
                <div className="space-y-3">
                  {files.map((file, idx) => (
                    <div key={file.id} className="flex items-center justify-between p-4 bg-surface-low rounded-lg group">
                      <div className="flex items-center gap-4 flex-1 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-on-surface-variant">{formatBytes(file.size)} • {file.pages} {t('merge.pages')}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => removeFile(file.id)} className="p-1 rounded-md text-on-surface-variant hover:text-red-500">
                          <X size={18} />
                        </button>
                      </div>
                    </div>
                ...
                {processing ? (
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 justify-center">
                    <Button variant="primary" onClick={downloadMerged} className="flex items-center gap-2">
                      <Download size={18} /> {t('merge.download')}
                    </Button>
